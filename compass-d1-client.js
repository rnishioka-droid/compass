(function () {
  ...
  class D1Query {
    ...
    offset(value) {
      this.spec.offset = Math.max(0, Number(value || 0));
      return this;
    }
  ...
  class PollChannel {
    ...
    subscribe(callback) {
      ...
      poll();
      // D1 rows_readを抑えるため、通常の変更検知は1分間隔にする。
      this.timer = window.setInterval(poll, 60000);
      return this;
    }
  }
    ...
    async request(path, options = {}) {
      ...
      try {
        ...
        const timeoutMessage = error?.name === "AbortError"
          ? `D1通信タイムアウト(${Math.round(REQUEST_TIMEOUT_MS / 1000)}秒)`
          : `D1通信失敗: ${error.message || error}`;
        throw apiError(
          `${timeoutMessage} / API=${this.url}${path} / origin=${window.location.origin} / token=${token ? "あり" : "なし"}`,
