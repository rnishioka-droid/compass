...
function parseCandidate(text, event, payload) {
  ...
  const validity = normalizeValidity(pick(fields, ["有効/無効", "有効／無効", "有効無効"]));
  return {
...
function normalizeValidity(value) {
  ...
  if (["○", "◯", "〇", "⚪", "⚪︎", "O", "o", "有効"].includes(text)) return "○";
  if (["×", "✕", "✖", "X", "x", "無効"].includes(text)) return "×";
  return "";
}
...
async function upsertCandidate(candidate, env) {
  ...
  const row = {
    ...
    raw: {
      ...
      invalidNew: candidate.invalidNew,
